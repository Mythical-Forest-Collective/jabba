...
if [ "$JABBA_VERSION" == "latest" ]; then
    ...
    JABBA_VERSION=$($JABBA_GET https://github.com/Mythical-Forest-Collective/jabba/releases/download/0.11.2/jabba-0.11.2-$OSTYPE-$OSARCH)
fi
...
case "$OSTYPE" in
    darwin*)
    BINARY_URL=https://github.com/Mythical-Forest-Collective/jabba/releases/download/${JABBA_VERSION}/jabba-${JABBA_VERSION}-darwin-amd64
    ;;
    linux*)
    case "$(uname -m)" in
        arm*|aarch*)
    ...
    BINARY_URL="https://github.com/Mythical-Forest-Collective/jabba/releases/download/${JABBA_VERSION}/jabba-${JABBA_VERSION}-linux-${OSARCH}"
    ;;
    cygwin*|msys*)
    OS_ARCH=$(echo 'echo %PROCESSOR_ARCHITECTURE% & exit' | cmd | tail -n 1 | xargs) # xargs used to trim whitespace
    if [ "$OS_ARCH" == "AMD64" ]; then
        BINARY_URL="https://github.com/Mythical-Forest-Collective/jabba/releases/download/${JABBA_VERSION}/jabba-${JABBA_VERSION}-windows-amd64.exe"
    else
        echo "OS_ARCH='$OS_ARCH' is not a valid architecture at this point."
        exit 1
    fi
...
please create a ticket at https://github.com/Mythical-Forest-Collective/jabba/issues."
    exit 1
    ;;
esac
...
If the problem persists - please create a ticket at https://github.com/Mythical-Forest-Collective/jabba/issues."
    exit 1
fi
...
{
echo "# https://github.com/Mythical-Forest-Collective/jabba"
...
(if you have any problems please report them at https://github.com/Mythical-Forest-Collective/jabba/issues)"
